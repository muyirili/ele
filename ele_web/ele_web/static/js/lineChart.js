function loadLineChart(elementId, dataset,yup,ydown) {
 ...
 //alert(names);
 //将X轴坐标设置为动态
 length=dataset[0].gdp.length;
 var max=(dataset[0].gdp[length-1])[0];
 //alert(max);
 var min=(dataset[0].gdp[0])[0];
 //alert(min);
 var xScale = d3.time.scale()
    .domain([new Date(min), new Date(min+11*500)])
 ...
 this.update = function (updateData,yup,ydown) {
  ...
  var currGdp = d3.max(updateData[i].gdp, function (d) {
   return d[1];
    ...
    //将X轴坐标设置为动态
    length=updateData[0].gdp.length;
    var max=(updateData[0].gdp[length-1])[0];
    //alert(max);
    var min=(updateData[0].gdp[0])[0];
    //alert(min);
    //alert(max);
    if(updateData[0].gdp.length<12)
    {
